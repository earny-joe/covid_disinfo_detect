...
def load_narrative_data():
    """
    Loads in narrative data with median embeddings for each narrative.
    """
    # get path to narrative data
    data_path = config.NARR_STORAGE_PATH / config.MEDIAN_NARR_FILENAME
...
def transform_median_embeddings(svd, df_narratives):
    """
    Given SVD model, transforms median embeddings
    """
    ...
    df_median_2d = gen_output_data(narratives, X_2d)

    return df_median_2d[['narrative', 'x', 'y']]
...
def main():
    ...
    """
    file_check = config.NARR_STORAGE_PATH / config.MEDIAN_NARR_DR_FILENAME
    ...
    if file_check.is_file():
        ...
        df_median_2d = transform_median_embeddings(model_dr, df_narratives)

        # save dataframe with 2d median embeddings to CSV
        df_median_2d.to_csv(
            f'{config.NARR_STORAGE_PATH}/{config.MEDIAN_NARR_DR_FILENAME}',
            index=False
        )
        print(
            '\nSaved 2D median narrative embeddings. File located at:\n',
            f'{config.NARR_STORAGE_PATH}/{config.MEDIAN_NARR_DR_FILENAME}',
            '\n'
        )
